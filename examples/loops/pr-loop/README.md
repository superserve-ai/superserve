...
| Tick is slow / times out                      | One tick is capped at **20 minutes**. Review steps are advisory, so a timeout remains visible in the logs without failing repository CI. Event-driven ticks review one PR; a manual `workflow_dispatch` sweeps _every_ open PR and can take much longer. Re-run from the Actions tab.              |
