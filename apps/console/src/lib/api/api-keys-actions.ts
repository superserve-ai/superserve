...
export async function ensureDashboardApiKeyAction() {
  const supabase = await createServerClient()
  const {
    data: { user },
  } = await supabase.auth.getUser()
  ...
  // Check if a dashboard key already exists (non-revoked)
  const { data: existing } = await admin
    .from("api_key")
    .select("id")
    .eq("team_id", teamId)
    .eq("name", "__console_dashboard__")
    .is("revoked_at", null)
    .limit(1)
    .single()

  if (existing) {
    // Key exists but we can't recover the plaintext — revoke and recreate
    await admin
      .from("api_key")
      .update({ revoked_at: new Date().toISOString() })
      .eq("id", existing.id)
