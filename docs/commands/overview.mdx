...
with Sandbox.create(name="data-analyzer") as sandbox:
...
<Note>
`AsyncSandbox` exposes the same API with awaitable methods — see the [Sandbox reference](/sdk-reference/sandbox#auto-cleanup).
</Note>
...
If the sandbox is `idle` when you call `run()`, the API auto-resumes it before executing. You don't need to call `resume()` manually.

## Cancel in-flight commands

Abort a command mid-execution by passing a cancellation token.
...
```python Python
# Python: use timeout_seconds to bound command duration.
