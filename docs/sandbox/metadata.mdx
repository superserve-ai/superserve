...
Metadata has API-side size and key-count limits; exceeding them returns a `400` from the API. Keys with reserved prefixes (`superserve.`, `_superserve`) are also rejected.
