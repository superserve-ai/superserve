...
<Info>
  **Prerequisites:** Python 3.12+, an agent script, and a [Superserve account](https://console.superserve.ai).
</Info>

## Option A: Zero-config deploy

The fastest path. No code changes, no config files.
...
<Steps>
  ...
  <Step title="Deploy your script">
    ```bash
    superserve deploy agent.py
    ```

    Superserve packages your code, detects dependencies from `requirements.txt` or `pyproject.toml`, and builds a sandboxed environment.
  </Step>
...
That's it. Your agent is deployed with an HTTPS endpoint, sandboxed execution, and persistent sessions. See [Zero-Config Deploy](/zero-config) for details on how it works.

---

## Option B: SDK integration

For agents that need fine-grained streaming control — status messages, structured metadata, and explicit response framing.
