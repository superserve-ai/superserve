...
It runs locally over stdio via `npx`, authenticates with your `SUPERSERVE_API_KEY`, and targets a sandbox per call by id. It's a thin wrapper over the [TypeScript SDK](/sdk-reference/sandbox), so the per-sandbox data-plane token never reaches the model.
...
- **Requires Node 18+.** The server runs on Node via `npx`.
