...
<Tabs>
  ...
  <Tab title="Codex">
    Add to `~/.codex/config.toml`. Codex also reads the server's `instructions` for cross-tool workflow guidance:
    ```toml
...
- **Egress is controllable.** Restrict outbound access with `allow_out`/`deny_out` on `sandbox_create` or `sandbox_update`, and audit what a sandbox actually reached with `sandbox_network_log`.
