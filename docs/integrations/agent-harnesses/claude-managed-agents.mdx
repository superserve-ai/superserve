...
description: "Execute Claude agent tool calls in isolated Firecracker microVMs with snapshot-based cold starts, pause/resume, and per-sandbox network isolation."
---
...
A self-hosted environment moves the execution layer to your infrastructure. The agent loop, model calls, and session history stay on Anthropic's servers. The filesystem and shell — where the agent's tools actually run — operate inside Superserve's Firecracker microVMs.
...
- **Superserve** provides per-session Firecracker microVMs — isolated sandboxes with their own filesystem, network namespace, and process tree.
...
Each session gets its own isolated VM. Filesystem changes persist across tool calls within a session. Between activity bursts, sandboxes can be paused to disk and resumed on demand.
...
Open the environment in the Console and click **Generate environment key**. Export both values on the host where your orchestrator will run:
...
Create your agent as you would for any Managed Agents setup — the agent doesn't know it will run on self-hosted infrastructure. That's determined per session by `environment_id`.

## Build the sandbox template

Build a Superserve template with Python and the Anthropic SDK pre-installed. This template becomes a Firecracker snapshot — every sandbox created from it boots in under a second with no image pull or package install.
...
<Tip>
For cost optimization, add a janitor thread that pauses sandboxes idle past a threshold. Superserve's `pause()` checkpoints the full VM state — memory, processes, and filesystem — to disk at zero compute cost. The next work item for that session calls `resume()` and picks up exactly where it left off, in under a second.
</Tip>
...
- **Sub-second cold starts.** Sandboxes restore from pre-built Firecracker snapshots. No image pull, no package install, no boot sequence at session time.
- **Pause and resume.** Checkpoint the full VM state to disk between turns. Resume picks up exactly where it left off — running processes, open files, in-memory state. Pay for compute only when the agent is actively working.
