...
Add or remove tools after creation:
...
batch_tool.register(another_tool, name="custom_name")

# Remove tools
batch_tool.unregister("old_tool")

# List available tools
print(batch_tool.list_tools())
```
...
```python
from rayai import BatchToolInput, BatchToolOutput
```
...
| Method | Description |
|--------|-------------|
| `register(tool, name=None)` | Register a tool (name auto-inferred) |
...
<CardGroup cols={1}>
  <Card title="Sandbox" icon="code" href="/tools/native/sandbox">
