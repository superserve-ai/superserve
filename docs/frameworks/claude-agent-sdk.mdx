...
The [Claude Agent SDK](https://platform.claude.com/docs/en/agent-sdk/overview) gives you tool use, conversation memory via `session_id`, and streaming out of the box. Combined with Superserve's durable workspace, the agent retains full context across turns and restarts - files it creates persist, and conversation history survives sandbox restarts.
