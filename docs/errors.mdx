...
import { NotFoundError, SandboxError } from "@superserve/sdk"

try {
  const info = await Sandbox.get("missing-id")
} catch (err) {
...
```python Python
from superserve import NotFoundError, SandboxError

try:
    info = Sandbox.get("missing-id")
