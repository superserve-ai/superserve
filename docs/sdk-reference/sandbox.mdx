...
Remove a secret binding by its environment-variable key. New processes no longer see the stand-in token, and a process already running loses the credential's access within about a minute. A paused sandbox applies the change on resume.
