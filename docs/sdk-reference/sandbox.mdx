...
Checkpoint the VM state to disk. Returns nothing.
...
A sandbox is `active` when running and `idle` after `pause()`. Deletion removes it entirely — subsequent API calls return `404`.
...
- `ConflictError` — wrong state (e.g., pausing an already-idle sandbox)
