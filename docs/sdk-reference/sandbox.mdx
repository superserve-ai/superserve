...
| `timeoutMs` / `timeout`                 | `number`  | With `wait`, how long to wait for `paused` before raising a timeout. Default 5 minutes.  |
| `pollIntervalMs` / `poll_interval_s`    | `number`  | With `wait`, how often to check the status. Default 1 second.                            |
