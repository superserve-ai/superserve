...
Block until the current build reaches a terminal status. Subscribes to the build's SSE log stream if `onLog` / `on_log` is provided; falls back to polling on network errors. Returns a refreshed [`TemplateInfo`](#templateinfo) with `status = "ready"`.
...
- `BuildError` if the build transitions to `failed` - `.code` is parsed from the `errorMessage` prefix (see [BuildSpec: Build error codes](/templates/build-spec#build-error-codes))
