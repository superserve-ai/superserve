...
Access files via the `sandbox.files` sub-module. Operations route directly to the data plane (`boxd-{id}.sandbox.superserve.ai`) using a per-sandbox access token. The SDK manages the token transparently, including rotation after `resume()`.
