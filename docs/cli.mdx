...
**Zero-config** — provide an entrypoint, no config file needed:
...
**Config mode** — reads `superserve.yaml`:
...
| Flag | Description |
|------|-------------|
| `ENTRYPOINT` | Script to run. Enables zero-config mode. |
| `--name` | Agent name (default: directory name) |
| `--port` | HTTP proxy mode — forward to this port |
