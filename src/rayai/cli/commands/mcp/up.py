...
The `rayai mcp up` command discovers and serves all MCP servers defined in the
mcp_servers/ directory. Each server file should call `rayai.serve_mcp()` which
registers the server for deployment.

Usage:
    rayai mcp up                    # Serve all MCP servers on port 8000
    rayai mcp up --port 8001        # Serve on custom port
    rayai mcp up --servers s1,s2    # Serve specific servers only
"""
...
def up(project_path: str, port: int, host: str, servers: str | None):
    ...
    Each server file should call rayai.serve_mcp() to register the server.

    Examples:
        rayai mcp up                    # Serve all servers
    ...
    # Set rayai mcp up mode before importing servers
    ...
    if imported_count == 0:
        ...
        click.echo("Create servers with: rayai create-mcp <name>")
        sys.exit(1)
    ...
    if not registered:
        ...
        click.echo("Ensure server.py files call rayai.serve_mcp()")
        sys.exit(1)
